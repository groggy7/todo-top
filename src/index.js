...
function Initialize() {
  ...
  if (Task.GetTaskList().length === 0) {
    ...
    new Task(
      "Call mom",
      "Call mom and tell her that I love her",
      formatDate(tomorrow),
      "High",
      "Personal"
    );
...
projects.innerHTML =
  '<span>My Projects</span><div class="add-project"><i class="fa-regular fa-plus fa-sm"></i></div>';
...
function RenderProjects() {
  ...
  Project.GetProjectList().forEach((project) => {
    const projectItem = createProjectItem({
      class: "project-item",
      projectIcon: "fa-bars-progress",
      text: project.name,
      id: project.uniqueKey,
      deleteIcon: "fa-trash",
    });
...
const cancelAddProjectButton = document.querySelector(
...
sidebar.addEventListener("click", (e) => {
  ...
  if (item && !item.classList.contains("add-task")) {
    ...
    const view = Object.keys(viewHandlers).find((key) =>
  ...
  tasks.forEach((task) => {
    ...
    taskItem.innerHTML = `
            ...
            <div class="task-content">
                ...
                <div class="task-description"><span>${
                  task.description
                }</span></div>
                <div class="task-priority"><span class="flag ${task.priority.toLowerCase()}"></span><span>${
      task.priority
    }</span></div>
                <div class="task-due-hour"><span>${
                  task.dueDate
                }</span><hr></div>
            </div>
    ...
    taskContent.addEventListener("click", function () {
      ...
      const titleField = updateTaskModal.querySelector(
      ...
      updateTaskForm.querySelector('input[name="task-title"]').value =
        task.title;
      updateTaskForm.querySelector('input[name="task-description"]').value =
        task.description;
      updateTaskForm.querySelector('input[name="task-due-date"]').value =
        task.dueDate;
      updateTaskForm.querySelector(
        ".select-selected"
      ).innerHTML = `<span class="flag ${task.priority.toLowerCase()}"></span> ${
        task.priority
      }`;
...
function loadInbox() {
  loadTasks(
    Task.GetTaskList().filter((task) => task.projectID.trim() === "none")
  );
}
...
function loadProjectTasks(projectName) {
  loadTasks(
    Task.GetTaskList().filter((task) => task.projectID.trim() === projectName)
  );
}
